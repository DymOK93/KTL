...
FMT_FUNC void assert_fail(const char* file, int line, const char* message) {
  // Use unchecked ktl::fprintf to avoid triggering another assertion when
  // writing to stderr fails
  DbgPrint("%s:%d: assertion failed: %s", file, line, message);
  // Chosen instead of ktl::abort to satisfy Clang in CUDA mode during device
  // code pass.
  ktl::terminate();
}
...
//  if (ktl::fwrite(full_message.data(), full_message.size(), 1, stderr) > 0)
//    ktl::fputc('\n', stderr);
//}
...
//                         FILE* stream) {
//  size_t written = ktl::fwrite(ptr, size, count, stream);
...
locale_ref::locale_ref(const Locale& loc) : locale_(&loc) {
  static_assert(ktl::is_same<Locale, ktl::locale>::value, "");
}

template <typename Locale>
Locale locale_ref::get() const {
  static_assert(ktl::is_same<Locale, ktl::locale>::value, "");
  return locale_ ? *static_cast<const ktl::locale*>(locale_) : ktl::locale();
}

template <typename Char>
FMT_FUNC auto thousands_sep_impl(locale_ref loc) -> thousands_sep_result<Char> {
  auto& facet = ktl::use_facet<ktl::numpunct<Char>>(loc.get<ktl::locale>());
  auto grouping = facet.grouping();
  auto thousands_sep = grouping.empty() ? Char() : facet.thousands_sep();
  return {ktl::move(grouping), thousands_sep};
}
template <typename Char>
FMT_FUNC Char decimal_point_impl(locale_ref loc) {
  return ktl::use_facet<ktl::numpunct<Char>>(loc.get<ktl::locale>())
      .decimal_point();
}
#else
template <typename Char>
...
// TODO: system_error
// FMT_FUNC ktl::system_error vsystem_error(int error_code, string_view
// format_str,
//                                         format_args args) {
//  auto ec = ktl::error_code(error_code, ktl::generic_category());
//  return ktl::system_error(ec, vformat(format_str, args));
//}
...
struct bits {
  static constexpr const int value =
      static_cast<int>(sizeof(T) * ktl::numeric_limits<unsigned char>::digits);
};
...
class fp {
 ...
 public:
  ...
  bool assign(Float d) {
    ...
    using limits = ktl::numeric_limits<Float>;
...
class bigint {
 ...
 public:
  ...
  void assign(const bigint& other) {
    ...
    ktl::copy(data, data + size, make_checked(bigits_.data(), size));
    exp_ = other.exp_;
  }
  ...
  friend int add_compare(const bigint& lhs1,
                         const bigint& lhs2,
                         const bigint& rhs) {
    int max_lhs_bigits = (ktl::max)(lhs1.num_bigits(), lhs2.num_bigits());
    ...
    int min_exp = (ktl::min)((ktl::min)(lhs1.exp_, lhs2.exp_), rhs.exp_);
  ...
  void square() {
    ...
    basic_memory_buffer<bigit, bigits_capacity> n(ktl::move(bigits_));
  ...
  void align(const bigint& other) {
    ...
    for (int i = num_bigits - 1, j = i + exp_difference; i >= 0; --i, --j)
      bigits_[j] = bigits_[i];
    ktl::uninitialized_fill_n(bigits_.data(), exp_difference, 0);
    exp_ -= exp_difference;
  }
...
inline uint64_t umul192_upper64(uint64_t x, uint128_wrapper y) noexcept {
...
inline uint32_t umul96_upper32(uint32_t x, uint64_t y) noexcept {
  return static_cast<uint32_t>(umul128_upper64(x, y));
}

// Computes middle 64 bits of multiplication of a 64-bit unsigned integer and a
// 128-bit unsigned integer.
inline uint64_t umul192_middle64(uint64_t x, uint128_wrapper y) noexcept {
...
inline bool divisible_by_power_of_2(uint32_t x, int exp) noexcept {
  FMT_ASSERT(exp >= 1, "");
  FMT_ASSERT(x != 0, "");
...
inline bool divisible_by_power_of_2(uint64_t x, int exp) noexcept {
  FMT_ASSERT(exp >= 1, "");
  FMT_ASSERT(x != 0, "");
...
inline bool divisible_by_power_of_5(uint32_t x, int exp) noexcept {
...
inline bool divisible_by_power_of_5(uint64_t x, int exp) noexcept {
...
template <int N>
bool check_divisibility_and_divide_by_pow5(uint32_t& n) noexcept {
...
inline int remove_trailing_zeros(uint32_t& n) noexcept {
#ifdef FMT_BUILTIN_CTZ
...
inline int remove_trailing_zeros(uint64_t& n) noexcept {
#ifdef FMT_BUILTIN_CTZLL
...
template <class T>
inline decimal_fp<T> shorter_interval_case(int exponent) noexcept {
...
int format_float(T value, int precision, float_specs specs, buffer<char>& buf) {
  static_assert(!ktl::is_same<T, float>::value, "");
  ...
  if (value <= 0) {  // <= instead of == to silence a warning.
    ...
    ktl::uninitialized_fill_n(buf.data(), precision, '0');
    return -precision;
  }
  ...
  *format_ptr = '\0';
  ...
  for (;;) {
    ...
    if (specs.format == float_format::fixed) {
      ...
      memmove(p, p + 1, to_unsigned(fraction_size));
...
// TODO format_system_error
// FMT_FUNC void format_system_error(detail::buffer<char>& out, int error_code,
//                                  const char* message) noexcept {
//  FMT_TRY {
//    auto ec = ktl::error_code(error_code, ktl::generic_category());
//    write(ktl::back_inserter(out), ktl::system_error(ec, message).what());
//    return;
//  }
//  FMT_CATCH(...) {}
...
FMT_FUNC ktl::ansi_string vformat(string_view fmt, format_args args) {
...
namespace detail {
// FMT_FUNC void print(ktl::FILE* f, string_view text) {
//#ifdef _WIN32
//  auto fd = _fileno(f);
//  if (_isatty(fd)) {
...
// FMT_FUNC void detail::vprint_mojibake(ktl::FILE* f, string_view format_str,
//                                      format_args args) {
//  memory_buffer buffer;
