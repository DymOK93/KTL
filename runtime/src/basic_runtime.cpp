...
namespace details {
driver_context
    driver_ctx{};  // Non-static due to usage in mini-filter runtime library
}
}  // namespace ktl::crt
...
KtlDriverEntry(DRIVER_OBJECT* driver_object,
  ...
  do {
    init_status = preload_initializer_registry::get_instance().run_all(
    ...
    if (auto& custom_drv_unload = details::driver_ctx.custom_unload;
        !custom_drv_unload) {
