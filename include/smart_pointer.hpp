...
class ref_counter_base {
 ...
 protected:
  virtual void destory_object() noexcept = 0;  //Функции должны обеспечивать
                                               //гарантии отсутствия исключений
  virtual void delete_this() noexcept = 0;

 private:
  atomic_counter_t inc_strong_refs() noexcept {
    return static_cast<atomic_counter_t>(InterlockedIncrementRelease(
        reinterpret_cast<volatile long*>(addressof(m_strong_counter))));
  }

  atomic_counter_t inc_weak_refs() noexcept {
    return static_cast<atomic_counter_t>(InterlockedIncrementRelease(
        reinterpret_cast<volatile long*>(addressof(m_weak_counter))));
  }

  atomic_counter_t dec_strong_refs() noexcept {
    return static_cast<atomic_counter_t>(InterlockedDecrementRelease(
        reinterpret_cast<volatile long*>(addressof(m_strong_counter))));
  }

  atomic_counter_t dec_weak_refs() noexcept {
    return static_cast<atomic_counter_t>(InterlockedDecrementRelease(
        reinterpret_cast<volatile long*>(addressof(m_weak_counter))));
  }

 private:
  atomic_counter_t m_strong_counter{1},
      m_weak_counter{
...
class ref_counter_with_scalar_delete
 ...
 protected:
  void destory_object() noexcept final { delete MyBase::exchange_ptr(nullptr); }
};
...
class ref_counter_with_array_delete
 ...
 protected:
  void destory_object() noexcept final {
...
class ref_counter_with_destroy_only
 ...
 protected:
  void destory_object() noexcept final { MyBase::exchange_ptr(nullptr)->~Ty(); }
};
...
class allocator_holder_base {
 public:
  ...
  Alloc& get_alloc() const noexcept { return m_alloc; }
...
class ref_counter_with_allocator_default_delete_itself
 ...
 public:
  ...
  ref_counter_with_allocator_default_delete_itself(Ty* ptr, const Alloc& alloc)
  ...
  void delete_this() noexcept final {
    auto alloc{
        move(MyAllocBase::get_alloc())};  //После вызова деструктора обращаться
                                          //к аллокатору нельзя!
    allocator_traits<Alloc>::destroy(alloc, this);
...
class PtrBase {
 ...
 protected:
  ...
  ConcretePtr& copy_construct_from(
      const PtrBase<U, ConcretePtr>& other) noexcept {
    static_assert(is_convertible_v<U*, Ty*>, "Can't convert value pointer");
    decref_if_not_null();
    exchange_value_ptr(other.m_value_ptr);
  ...
  template <class U>
  ConcretePtr& move_construct_from(PtrBase<U, ConcretePtr>&& other) noexcept {
    static_assert(is_convertible_v<U*, Ty*>, "Can't convert value pointer");
    decref_if_not_null();
    exchange_value_ptr(other.exchange_value_ptr(nullptr));
  ...
  void swap(PtrBase& other) noexcept {
    auto* value_ptr{m_value_ptr};
    auto* ref_counter{m_ref_counter};
...
class weak_ptr : public details::PtrBase<Ty, weak_ptr<Ty> > {
 ...
 public:
  ...
  bool expired() noexcept { return MyBase::use_count() == 0; }
...
class shared_ptr : public details::PtrBase<Ty, shared_ptr<Ty> > {
 ...
 public:
  ...
  shared_ptr(U* ptr, Dx&& deleter, custom_deleter_tag)
      : MyBase(
            ptr,
  ...
  shared_ptr(U* ptr, Alloc&& alloc, custom_allocator_tag)
      : MyBase(ptr,
  ...
  template <class Dx>
  shared_ptr(nullptr_t, Dx&& deleter, custom_deleter_tag)
      : MyBase(
            nullptr,
            make_special_ref_counter<details::ref_counter_with_custom_deleter>(
                nullptr,
  ...
  template <class Alloc>
  shared_ptr(nullptr_t, Alloc&& alloc, custom_allocator_tag)
      : MyBase(nullptr,
               make_special_ref_counter<
                   details::ref_counter_with_allocator_default_delete_itself>(
                   nullptr,
  ...
  shared_ptr& operator=(shared_ptr&& other) {
    if (addressof(other) != this) {
      MyBase::move_construct_from(other);
    }
    return *this;
  }
  ...
  shared_ptr& operator=(unique_ptr<U, Dx>&& uptr) {
    auto* ptr{uptr.release()};
    MyBase::move_construct_from(
        MyBase(ptr, make_special_ref_counter_with_deleter<
                        details::ref_counter_with_custom_deleter>(
                        ptr, move(uptr.get_deleter()))));
  }
 ...
 protected:
  ...
  template <class Ty, class... Types>
  friend shared_ptr<Ty> make_shared(Types&&... args);
 ...
 private:
  ...
  make_special_ref_counter(U* ptr, Destroyer&& destroyer) noexcept(
      is_nothrow_constructible<Destroyer, Destroyer>) {
    return new (nothrow) RefCounter<U, remove_reference_t<Destroyer> >(
        ptr,
        forward<Destroyer>(destroyer));  //Конструирование с нулевым указателем
                                         //и кастомным Deleter'ом допускается
  }
};
template <class Ty>
...
shared_ptr<Ty> make_shared(Types&&... args) {
  ...
  new (memory_block) ref_counter_t(object_ptr);  //Конструирование ref_counter'a
  shared_ptr<Ty> sptr(object_ptr, static_cast<ref_counter_t*>(memory_block));
...
shared_ptr<Ty> allocate_shared(Alloc&& alloc, Types&&... args) {
  ...
  Ty* object_ptr{reinterpret_cast<Ty*>(static_cast<byte*>(memory_block) +
                                       sizeof(ref_counter_t))};
  ref_counter_t* ref_counter{allocator_traits<Alloc>::construct(
      alloc, memory_block)};  //Нельзя сразу передать alloc, т.к. он может быть
                              //перемещён
  ref_counter->get_alloc() = forward<Alloc>(alloc);
